## 智能体数据安全审计系统
多个 AI Agent 共享同一数据平台，会带来很多数据不可信的安全风险。于是我们通过分布式管理密钥，远程数据可持有证明开发了智能体数据安全审计系统，从而去确保Agent可以基于可信数据进行分析和决策。
本系统基于我们已发表的一篇sci论文核心思想实现，并在工程层面进行了扩展与优化，目前软件著作权正在申请中。
